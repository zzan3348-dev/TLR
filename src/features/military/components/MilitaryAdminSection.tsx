...
export function MilitaryAdminSection({ data, onReload, onError }: { data: MilitaryAdminData; onReload: () => Promise<void>; onError: (message: string) => void }) {
  ...
  return (
    <section className="directorate-military" aria-labelledby="directorate-military-title">
      <header>
        <div><h2 id="directorate-military-title">군사 데이터·전쟁 관제</h2></div>
          ...
          {target.kind === "IDEOLOGY" && ideology ? (
            ...
            <>
              <div className="directorate-military__fields">
                ...
                <label>설정 상태<select value={catalog.configuration_status} onChange={(event) => setCatalog({ ...catalog, configuration_status: event.target.value as Catalog["configuration_status"] })}><option value="PARTIAL">일부 설정</option><option value="READY">준비됨</option><option value="DISABLED">비활성</option></select></label>
      ...
      <section className="directorate-military__command">
        <header><h3>전쟁·전선·점령 관제</h3></header>
      ...
      <section className="directorate-military__review"><header><h3>작전 판정 대기열</h3></header>{data.actions.length === 0 ? <p>판정을 기다리는 작전이 없습니다.</p> : data.actions.map((actionRow) => {
        const form = resolution[actionRow.id] ?? BLANK_RESOLUTION;
