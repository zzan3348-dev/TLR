...
export function AdminMembershipSection() {
  ...
  return <section className="directorate-diplomacy directorate-membership" aria-labelledby="directorate-membership-title"><header><div><h2 id="directorate-membership-title">디스코드 관리자 등록</h2></div></header><div className="directorate-preview__body"><label>디스코드 사용자명<input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="guitar_hero0405" /></label><button type="button" disabled={busy || !username.trim()} onClick={() => void register()}>{busy ? "등록 중…" : "관리자 등록"}</button>{message ? <p role="status">{message}</p> : null}</div></section>;
}
