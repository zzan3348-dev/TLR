import { uiIconAssets } from "../data/uiAssets";
import "../features/management/management.css";
import { managementLabel } from "../features/management/labels";
import { useSessionPreference } from "../features/management/useSessionPreference";
...
const MANAGEMENT_NAV: Array<{ group: string; items: Array<{ id: ManagementView; label: string; hint: string }> }> = [
  { group: "운영", items: [{ id: "dashboard", label: "세계 운영", hint: "상태와 처리 대기" }, { id: "world-control", label: "세계시간", hint: "날짜 진행·턴 경계" }] },
  { group: "콘텐츠", items: [{ id: "event-studio", label: "이벤트", hint: "제작·예약·게시" }, { id: "decision-studio", label: "디시전", hint: "실행 정의 감사" }] },
  { group: "세계", items: [{ id: "provinces", label: "영토·지역", hint: "지역 그룹" }, { id: "capitals", label: "수도", hint: "지도 기준점" }, { id: "economy", label: "경제·무역", hint: "데이터와 계약" }, { id: "diplomacy", label: "외교", hint: "제안 검토" }] },
...
const NAV_ICONS: Record<ManagementView, string> = {
  dashboard: uiIconAssets.menu.fit, "world-control": uiIconAssets.worldControl.advanceTime,
  "event-studio": uiIconAssets.menu.decisions, "decision-studio": uiIconAssets.menu.politics,
  provinces: uiIconAssets.menu.layers, capitals: uiIconAssets.menu.capital,
  economy: uiIconAssets.menu.economy, diplomacy: uiIconAssets.menu.diplomacy,
  military: uiIconAssets.worldControl.armyMap, intelligence: uiIconAssets.menu.intelligence,
  research: uiIconAssets.menu.research, system: uiIconAssets.menu.politics,
};
...
export function DirectoratePanel({ onBackToTitle }: DirectoratePanelProps) {
  ...
  const [activeView, setActiveView] = useSessionPreference<ManagementView>("view", "dashboard", MANAGEMENT_NAV.flatMap((group) => group.items.map((item) => item.id)));
  ...
  const upcoming = (researchData?.projects ?? []).filter((project) => project.status === "ACTIVE" && project.scheduled_completion_world_date).sort((a, b) => (a.scheduled_completion_world_date ?? "").localeCompare(b.scheduled_completion_world_date ?? "")).slice(0, 6);
  ...
  return (
    <main className="directorate-page">
      <header className="directorate-page__header">
        <div><p>THE LONG REVOLUTION</p><h1>세계 운영실</h1></div>
        <button type="button" onClick={() => { if (!hasUnsavedContent || window.confirm("진행 중이거나 저장하지 않은 작업이 있습니다. 콘솔을 닫을까요?")) onBackToTitle(); }}>닫기</button>
      </header>
      <section className="directorate-page__status">
        <span className="directorate-page__status-light" /> 관리자 연결됨
      </section>
      <div className="management-console">
        <aside className="management-console__nav" aria-label="관리자 콘솔 메뉴">
          <div><strong>운영 메뉴</strong><button type="button" onClick={() => setPaletteOpen(true)}>검색 <kbd>Ctrl K</kbd></button></div>
          {MANAGEMENT_NAV.map((group) => <section key={group.group}><h2>{group.group}</h2>{group.items.map((item) => <button type="button" data-active={activeView === item.id} aria-current={activeView === item.id ? "page" : undefined} title={item.hint} onClick={() => setActiveView(item.id)} key={item.id}><img src={NAV_ICONS[item.id]} alt="" /><strong>{item.label}</strong></button>)}</section>)}
        </aside>
        <section className="management-console__main">
          {feedback ? <div className="management-feedback" role="status">{feedback}<button type="button" onClick={() => setFeedback("")}>닫기</button></div> : null}
          {activeView === "dashboard" ? <section className="management-dashboard"><header><div><h2>세계 운영</h2></div><div className="management-dashboard__clock"><span>세계시간</span><strong>{worldDate || "확인 중"}</strong><span>현재 턴</span><strong>{worldControlData?.turn.configured ? `${worldControlData.turn.number}턴` : "스케줄 미설정"}</strong></div></header>{loadFailures.length ? <div className="management-load-warning" role="alert"><strong>불러오지 못한 관제 영역 {loadFailures.length}개</strong><span>{loadFailures.join(" · ")}</span><button type="button" onClick={() => void loadQueue()}>다시 불러오기</button></div> : null}<h3>처리 필요</h3><div className="management-dashboard__metrics"><button type="button" onClick={() => setActiveView("military")}><span>군사작전 판정</span><strong>{militaryData?.actions.filter((action) => action.status === "SUBMITTED").length ?? 0}</strong></button><button type="button" onClick={() => setActiveView("intelligence")}><span>첩보 검토</span><strong>{intelligenceData?.operations.filter((operation) => operation.admin_review_status === "PENDING").length ?? 0}</strong></button><button type="button" onClick={() => setActiveView("research")}><span>연구 심사</span><strong>{researchData?.projects.filter((project) => ["SUBMITTED", "UNDER_REVIEW", "APPROVED"].includes(project.status)).length ?? 0}</strong></button><button type="button" onClick={() => setActiveView("diplomacy")}><span>외교 요청</span><strong>{queue.length}</strong></button><button type="button" onClick={() => setActiveView("economy")}><span>무역 검토</span><strong>{tradeQueue.length}</strong></button><button type="button" onClick={() => setActiveView("world-control")}><span>세계시간 요청</span><strong>{worldControlData ? worldControlData.counts.advance + worldControlData.counts.hold : 0}</strong></button></div><section className="management-upcoming"><h3>다가오는 연구 일정</h3>{upcoming.length ? upcoming.map((project) => <button type="button" key={project.id} onClick={() => setActiveView("research")}><time>{project.scheduled_completion_world_date}</time><span>{countryName(project.country_key)} · {project.title}</span></button>) : <p>{researchData ? "예정된 연구 완료 없음" : "연구 일정을 불러오지 못했습니다."}</p>}</section><div className="management-dashboard__quick"><h3>빠른 작업</h3><button type="button" onClick={() => setActiveView("world-control")}>세계시간 진행</button><button type="button" onClick={() => setActiveView("event-studio")}>이벤트 제작·예약</button><button type="button" onClick={() => setActiveView("provinces")}>지역 편집</button><button type="button" onClick={() => setActiveView("system")}>플레이 테스트</button></div></section> : null}
          <div hidden={activeView !== "event-studio"}><ContentStudio onPendingChange={setHasUnsavedContent} /></div>
          ...
          {activeView === "world-control" ? worldControlData ? <><WorldControlAdminSection data={worldControlData} onReload={loadQueue} onError={setQueueError} /><section className="management-turn-separation"><article><h3>세계날짜</h3><strong>{worldControlData.worldDate}</strong><p>이벤트·연구·첩보·건조의 달력 기준</p></article><article><h3>턴 관리</h3><strong>{worldControlData.turn.configured ? `${worldControlData.turn.number}턴 · ${managementLabel(worldControlData.turn.status)}` : "턴 스케줄 미설정"}</strong><p>{worldControlData.turn.configured ? `${worldControlData.turn.startWorldDate} — ${worldControlData.turn.endWorldDate ?? "종료 경계 미설정"}` : "날짜에서 턴을 임의 추정하지 않습니다."}</p></article></section></> : <section className="management-empty-state"><strong>세계시간 데이터를 불러오지 못했습니다.</strong><button type="button" onClick={() => void loadQueue()}>다시 불러오기</button></section> : null}
      ...
      <section className="directorate-diplomacy" aria-labelledby="directorate-economy-title">
        <header>
          <div><h2 id="directorate-economy-title">경제·무역</h2></div>
          <strong>경제 입력 {economies.length}국 · 계약 {tradeAgreements.length}건</strong>
        </header>
        <div className="directorate-diplomacy__queue">
          {economies.map((economy) => <article key={economy.country_key}><div><h3>{countryName(economy.country_key)}</h3><p>GDP {economy.gdp ?? "미설정"} · 생산능력 {economy.base_production_capacity ?? "미설정"}</p></div></article>)}
          {tradeAgreements.map((agreement) => (
            <article key={agreement.id}>
              <div><small>{managementLabel(agreement.status)} · {agreement.starts_world_date} — {agreement.ends_world_date}</small><h3>{countryName(agreement.country_a_key)} ↔ {countryName(agreement.country_b_key)}</h3><p>{agreement.lines.length}개 항목 · 다음 정산 {agreement.next_settlement_world_date ?? "없음"}</p></div>
      ...
      <section className="directorate-diplomacy" aria-labelledby="directorate-trade-title">
        <header>
          <div><h2 id="directorate-trade-title">무역 처리 대기</h2></div>
      ...
      {activeView === "diplomacy" ? <section className="directorate-diplomacy" aria-labelledby="directorate-diplomacy-title">
        <header>
          <div><h2 id="directorate-diplomacy-title">외교 처리 대기</h2></div>
      ...
      {paletteOpen ? <div className="management-palette" role="dialog" aria-modal="true" aria-label="관리자 명령 검색"><div><header><strong>메뉴 검색</strong><button type="button" onClick={() => setPaletteOpen(false)}>×</button></header><input aria-label="메뉴 검색" placeholder="메뉴 이름 검색" value={menuQuery} onChange={(event) => setMenuQuery(event.target.value)} />{MANAGEMENT_NAV.flatMap((group) => group.items).filter((item) => item.label.includes(menuQuery)).map((item) => <button type="button" key={item.id} onClick={() => { setActiveView(item.id); setPaletteOpen(false); }}><img src={NAV_ICONS[item.id]} alt="" /><strong>{item.label}</strong></button>)}</div></div> : null}
    </main>
  );
}
