...
export default async function handler(request: ApiRequest, response: ApiResponse): Promise<void> {
  ...
  if (!env) {
    response.status(503).json({ error: "RESEARCH_SERVER_NOT_CONFIGURED" });
    return;
  }
  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      const failed = [projects, economies, settings].find((result) => result.error);
      if (failed?.error) throw failed.error;
      response.status(200).json({ worldDate, projects: projects.data ?? [], economies: economies.data ?? [], settings: settings.data });
      return;
    }
    ...
    if (["REJECT", "FORCE_COMPLETE", "CANCEL"].includes(action)) {
      ...
      if (status === "COMPLETED") update.completed_world_date = worldDate;
      if (status === "CANCELLED") update.cancelled_world_date = worldDate;
      ...
      await admin.from("research_audit_logs").insert({
    ...
    if (action === "ADJUST_END_DATE") {
      ...
      if (!projectId || !/^\d{4}-\d{2}-\d{2}$/.test(completionDate)) {
      ...
      await admin.from("research_audit_logs").insert({
        ...
        actor_subject: session.sub,
        action,
        details: { completionDate, note },
        world_date: worldDate,
      });
      response.status(200).json({ ok: true, scheduledCompletionWorldDate: completionDate });
      return;
    }
    if (action === "ADJUST_POINTS") {
      const countryKey = cleanText(body.countryKey, 40);
      const amount = typeof body.amount === "number" && Number.isFinite(body.amount) ? body.amount : null;
      if (!countryKey || amount === null) {
        response.status(400).json({ error: "INVALID_RESEARCH_ADJUSTMENT" });
        return;
      }
      ...
      const balance = Math.max(0, Number(row?.research_points ?? 0) + amount);
      const { error } = await admin
        .from("country_economies")
        .upsert({ country_key: countryKey, research_points: balance }, { onConflict: "country_key" });
      if (error) throw error;
      await admin.from("research_admin_adjustments").insert({
      ...
      await admin.from("research_audit_logs").insert({
        ...
        actor_subject: session.sub,
        action,
        details: { amount, balance, note },
        world_date: worldDate,
      });
      response.status(200).json({ ok: true, balance });
      return;
    }
