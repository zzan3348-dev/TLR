...
export default async function handler(request: ApiRequest, response: ApiResponse): Promise<void> {
  ...
  try {
    ...
    response.status(200).json({
      ...
      balance: Number(mergedEconomy?.research_points ?? 0),
      incomePerPeriod: Number((Array.isArray(settled.data) ? settled.data[0]?.income_per_period : settled.data?.income_per_period) ?? mergedEconomy?.research_income_per_period ?? 0),
      researchCapacity: Number(mergedEconomy?.research_capacity ?? 0),
